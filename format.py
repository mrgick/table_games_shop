...
def find_dirs() -> list[str]:
    """Получение всех папок, не в .gitignore"""
    exclude_dirs = []
...
def find_files(dirs: list) -> list[Path]:
    """Получение всех файлов по папкам рекурсивно и в текущей директории нерекурсивно"""
...
def run():
    ...
    if args.fix is None:
        print("Ruff fixing")
        subprocess.run(["ruff", *files, "--fix"], stdout=stdout)
